...
# We look for the part starting with 'for idx in tqdm(img_ids):'
# and ending at 'if not results:'
start_idx = eval_code.find("    for idx in tqdm(img_ids):")
end_idx = eval_code.find("    if not results:")

if start_idx != -1 and end_idx != -1:
    new_loop = """    for idx in tqdm(img_ids):
...
"""
    eval_code = eval_code[:start_idx] + new_loop + eval_code[end_idx:]
